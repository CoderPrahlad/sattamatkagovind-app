...
export async function POST(request: Request) {
  try {
    // Verify user is authenticated (any logged-in user can upload)
    ...
    // Sanitize category - only allow safe directory names
    const safeCategory = category.replace(/[^a-zA-Z0-9_-]/g, '') || 'screenshots';
